...
function StatCard({ icon, title, value, subtitle }) {
  return (
    ...
    >
      ...
      <h3 className="mt-2 text-5xl font-black tracking-tight text-white">{value}</h3>
...
function DoctorDashboard() {
  ...
  const doctorName = JSON.parse(localStorage.getItem("user") || "{}")?.name || "Doctor";
  ...
  const fetchRecords = async () => {
    try {
      ...
      setError(err.response?.data?.message || "Failed to load records. Check your connection.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredRecords = useMemo(() => {
    return selectedPatient === "all"
      ...
      : records.filter(
          (record) =>
            record.patientWallet?.toLowerCase() === selectedPatient.toLowerCase()
        );
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="relative z-10 p-4 md:p-8">
        <nav className="mb-8 flex flex-col gap-4 rounded-3xl border border-white/10 bg-white/[0.06] p-5 shadow-2xl shadow-cyan-950/20 backdrop-blur-xl md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              className="flex h-12 items-center gap-2 rounded-2xl border border-red-300/20 bg-red-500/10 px-5 font-black text-red-200 hover:bg-red-500/20"
            >
              <FiLogOut />
              Logout
            </button>
        ...
        <header className="mb-8">
          ...
          <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-300 md:text-base">
            View patient records only when access is granted, verify blockchain-linked files,
            and download encrypted medical data through authorized permissions.
          </p>
        </header>
          ...
          <div className="rounded-3xl border border-emerald-300/15 bg-emerald-400/10 p-5 backdrop-blur-xl">
            <div className="mb-3 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-emerald-100/80">Smart contract permissions active</p>
              </div>
            </div>
        ...
        >
          {loading ? (
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-2xl font-black text-white">No records available</p>
              <p className="mt-2 text-sm text-slate-400">
                No patients have granted access yet, or no records match this filter.
              </p>
            </div>
          ) : (
                ...
                <tbody>
                  {filteredRecords.map((record, index) => (
                    ...
                    >
                      ...
                      <td className="p-4 font-bold text-white">{record.fileName}</td>
                      <td className="p-4 font-mono text-xs text-slate-300">
                        {record.patientWallet
                          ? `${record.patientWallet.slice(0, 8)}...${record.patientWallet.slice(-6)}`
                          : "-"}
                      </td>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-xl">
      ...
      >
        <div className="flex items-center justify-between border-b border-white/10 p-5">
          <div>
            <h3 className="text-xl font-black text-white">{record.fileName}</h3>
        ...
        <div className="flex-1 overflow-auto bg-slate-900/70 p-5">
          {loading && (
            <div className="py-16 text-center text-slate-300">Loading preview...</div>
          )}

          {error && (
            <div className="py-16 text-center font-bold text-red-200">{error}</div>
          )}
