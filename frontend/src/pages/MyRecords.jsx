...
function MyRecords() {
  ...
  const categories = useMemo(() => {
    const unique = new Set(records.map((record) => record.recordType).filter(Boolean));
    return ["all", ...Array.from(unique)];
  }, [records]);

  const filteredRecords = records.filter((record) => {
    const text = `${record.recordType || ""} ${record.fileName || ""} ${record.ipfsHash || ""}`.toLowerCase();
      ...
      <div className="relative z-10 flex min-h-screen">
        <aside className="hidden w-80 border-r border-white/10 bg-slate-950/70 p-6 backdrop-blur-2xl lg:block">
          ...
          <nav className="space-y-3">
            ...
            <SidebarLink to="/my-records" active icon={<FiFileText />} label="My Records" />
        ...
        <section className="flex-1 p-4 md:p-8">
          <header className="mb-8 rounded-3xl border border-white/10 bg-white/[0.06] p-6 shadow-2xl shadow-cyan-950/20 backdrop-blur-xl">
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">
              Search encrypted records, inspect IPFS hashes, verify blockchain transactions,
              and download authorized files.
            </p>
          </header>
          ...
          >
            {loading ? (
              <div className="p-10 text-center text-slate-300">Loading records...</div>
                ...
                <table className="w-full min-w-[1050px]">
                  ...
                  <tbody>
                    {filteredRecords.map((record, index) => (
                      ...
                      >
                        <td className="p-4 font-black text-white">{record.recordType}</td>
                        ...
                        <td className="p-4 text-slate-300">
                          {new Date(record.uploadedAt || record.createdAt).toLocaleString()}
                        </td>
                        <td className="p-4">
                          <button
